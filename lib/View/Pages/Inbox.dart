...
class _InboxState extends State<Inbox> {
  ...
  Future getDataGrid() async {
    var response = await http.get(Uri.parse(Uri.encodeFull('https://olla.ws/api/customer/v1/brodcast')), headers: {
  ...
  void initState() {
    super.initState();
    getDataGrid().whenComplete(() => setState(() {
          loading = true;
        }));
      ...
      appBar: AppBar(
        ...
        title: Container(
          ...
          child: Center(
              child: Text(
            "Inbox",
            style: TextStyle(color: Colors.black, fontSize: 16, fontFamily: 'Inter', fontWeight: FontWeight.bold),
          )),
        ),
      ),
      body: SafeArea(
                                  ...
                                  child: Column(
                                    children: [
                                      Container(
                                        height: 200.0,
                                        child: Ink.image(
                                          image: NetworkImage(data![index]['banner_foto']),
                                          fit: BoxFit.cover,
                                        ),
                                      ),
                                      Container(
